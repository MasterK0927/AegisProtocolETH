...
export function RentalModal({ isOpen, onClose, agent }: RentalModalProps) {
  ...
  const [renterApiKeys, setRenterApiKeys] = useState<Record<string, string>>(
    {}
  );
  ...
  const handleWalletConnect = async () => {
    ...
    // Show API key modal instead of proceeding directly to rental
    setShowWalletSelection(false);
    setShowApiKeyModal(true);
  };

  const handleApiKeysSubmitted = async (apiKeys: Record<string, string>) => {
    ...
    try {
      ...
      // Store API keys in session storage for the chat
      sessionStorage.setItem(
        `agent_${agent.tokenId}_api_keys`,
